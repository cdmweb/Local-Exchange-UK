...
{
	header("location:" . SERVER_PATH_URL . $GLOBALS['SITE_SECTION_URL'][$redir_type]."?item=".$redir_item);
	exit;
}

if (isset($redir_type))	// $item not specified
{
	header("location:" . SERVER_PATH_URL .$GLOBALS['SITE_SECTION_URL'][$redir_type]);
	exit;
}
