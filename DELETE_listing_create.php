...
//
$cUser->MustBeLoggedIn();
// todo: should track admin actions
...
if($_REQUEST["mode"] == "admin") {  // Administrator is creating listing for another member
	...
	if (isset($_REQUEST["member_id"])) {
		...
		$ids->LoadMemberGroup();
		$form->addElement("select", "member_id", "For which member?", $ids->MakeIDArray());
	}
...
$form->addElement('text', 'title', 'Title', array('size' => 30, 'maxlength' => 60));
...
$form->addRule('category', 'Choose a category', 'verify_category');
...
function process_data ($values) {
	...
	if($created) {
		$list .= "Listing created. Create <A HREF=listing_create.php?type=".$_REQUEST["type"]."&mode=".$_REQUEST["mode"]."&member_id=".$member->member_id.">another</A>?";	
	} else {
